...
public class ItemParser
{
	 ...
	 * @throws APIRestGeneratorException with an occurred exception
	 */
	protected String getAlias(final String keyName, final JsonNode keyNode) throws APIRestGeneratorException
	{
		// Firstly, prevent the old 'alias' parameter was removed
		final String aliasObsolete = ParserUtil.getNodeValueField(keyName, keyNode,  ConstantsInputParser.SW_COMMON_ALIAS_OBSO, false) ;
		
		if (aliasObsolete != null && !aliasObsolete.isEmpty())
		{
			final String errorString = "'" + ConstantsInputParser.SW_COMMON_ALIAS_OBSO + "' property is obsolete. " +
									   "Please, use '" + ConstantsInputParser.SW_COMMON_ALIAS_BBVA + "' instead of that." ;
			
			ItemParser.LOGGER.error(errorString) ;
	    	throw new APIRestGeneratorException(errorString) ;
		}
		
		// Secondly, get the expected value if exists
		return ParserUtil.getNodeValueField(keyName, keyNode,  ConstantsInputParser.SW_COMMON_ALIAS_BBVA, false) ;
	}
