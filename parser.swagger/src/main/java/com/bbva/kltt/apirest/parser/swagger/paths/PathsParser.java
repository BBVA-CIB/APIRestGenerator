...
import com.bbva.kltt.apirest.parser.swagger.operations.OperationParser;
import com.bbva.kltt.apirest.parser.swagger.parameters.ParametersPathParser;
...
public class PathsParser
{
	...
	private void parsePathOpObject(final String keyPath, final Path path, final JsonNode pathNode) throws APIRestGeneratorException
	{
		...
		while (keyPaths.hasNext())
		{
			...
			final boolean isPathOpSetTwo   = ConstantsInputParser.SW_PATH_OP_OPTIONS.equalsIgnoreCase(pathOpKey) ||
											 ConstantsInputParser.SW_PATH_OP_PATCH.equalsIgnoreCase(pathOpKey) 	 ||
										     ConstantsInputParser.SW_PATH_OP_POST.equalsIgnoreCase(pathOpKey)  	 ;
										     
			
			final boolean isPathOpSetThree = ConstantsInputParser.SW_PATH_OP_PUT.equalsIgnoreCase(pathOpKey)     ;
			
			if (isPathOpSetOne || isPathOpSetTwo || isPathOpSetThree)
			{
