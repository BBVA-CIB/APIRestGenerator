...
import com.bbva.kltt.apirest.core.parsed_info.parameters.ParameterFactory;
...
public abstract class ParametersBaseParser extends ItemParser
{
    ...
    protected Parameter parseParameterObject(final String keyName, final JsonNode keyNode) throws APIRestGeneratorException
    {
        ...
        final String inParameter  = ParserUtil.getNodeValueField(keyName, keyNode, ConstantsInputParser.SW_PARAM_SUBSC_IN,    true) ;
        ...
        final String inValue      = this.getParamType(inParameter) ;
        final String autoInjected = ParserUtil.getNodeValueField(keyName, keyNode, ConstantsInputParser.SW_PARAM_AUTOIN_BBVA, false) ;

        Parameter parameter = null;
        Item item = null;

        if (this.parameterFactory.isBodyParameter(inParameter, ConstantsInputParser.SW_PARAM_IN_BODY))
        {
            ...
            parameter = this.parameterFactory.createNewParameter(inValue, name, alias, description, required, type, autoInjected);
        ...
        // Inheritance way for the "required" parameter due to the specific behavior in the parameters
        item.setRequired(parameter.isRequired()) ;
