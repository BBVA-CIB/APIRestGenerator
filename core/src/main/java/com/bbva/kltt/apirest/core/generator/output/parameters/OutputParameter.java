...
public abstract class OutputParameter
{
	...
	/**
	 * This parameter should be the alias but there are other cases (when alias is not filled) then we need to use 'name' 
	 * @return the official parameter name (alias or name) to be send via Web
	 */
	public String getParameterOfficialName()
	{
		String outcome = this.item.getName() ;
		
		if (this.item.getAlias() != null && !this.item.getAlias().isEmpty())
		{
			outcome = this.item.getAlias() ;
		}
