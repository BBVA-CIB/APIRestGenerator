...
{
	/** Logger of the class */
	private final static Logger LOGGER = LoggerFactory.getLogger(FilesUtility.class);
	...
	public static String convertPackageToPath(final String basePath, final String packageName)
	{
		if (packageName != null)
		{
			return basePath + File.separator + FilesUtility.convertPackageToPath(packageName);
		}
		else
		{
			return basePath;
	...
	private static void closeStreams(final InputStream inputStream, final OutputStream outputStream) throws APIRestGeneratorException
	{
		if (inputStream != null)
    	{
    		...
    		{
    			final String errorString = "IOException while closing the input stream after copied the file" ;
    	...
    	if (outputStream != null)
    	{
    		...
    		{
				final String errorString = "IOException while closing the output stream after copied the file" ;
	...
	public static String loadFileContentFromClasspath(final String location)
	{
		...
		if (inputStream == null)
		{
			inputStream = FilesUtility.class.getClassLoader().getResourceAsStream(location) ;
		}
