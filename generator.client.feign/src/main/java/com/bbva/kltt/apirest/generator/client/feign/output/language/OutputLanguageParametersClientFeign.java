...
package com.bbva.kltt.apirest.generator.client.feign.output.language;
...
import com.bbva.kltt.apirest.generator.client.feign.output.parameters.OutputParameterClientFeign;
import com.bbva.kltt.apirest.generator.client.feign.output.parameters.OutputParametersClientFeign;
 ...
 * ------------------------------------------------
 */
public class OutputLanguageParametersClientFeign extends OutputLanguageParametersJava
{
	/**
	 * Public constructor
	 ...
	 */
	public OutputLanguageParametersClientFeign(final ParsedInfoHandler parsedInfoHandler, final IOutputLanguageItems outputLanguageItems)
	{
	...
	{
		return new OutputParametersClientFeign(this.getOutputLangSeparators(), outputParamList) ;
	}
	...
	public OutputParameter createNewOutputParameter(final Parameter parameter)
	{
		...
		return new OutputParameterClientFeign(parameter.getItem(), parameter.getDescription(), type, parameter.getName(), annotation) ;
	}
	...
	private String getAnnotation(final Parameter parameter)
	{
		...
		{
			annotationName = ConstantsOutputClientFeign.FEIGN_ANN_REQUEST_BODY ;
		}
		else if (parameter instanceof ParameterFormData || parameter instanceof ParameterQuery)
		{
			annotationName = ConstantsOutputClientFeign.FEIGN_ANN_REQUEST_PARAM ;
		}
		else if (parameter instanceof ParameterHeader)
		{
			annotationName = ConstantsOutputClientFeign.FEIGN_ANN_REQUEST_HEADER ;
		}
		else if (parameter instanceof ParameterPath)
		{
			annotationName = ConstantsOutputClientFeign.FEIGN_ANN_PATH_VARIABLE ;
		}
		
		// Add the annotation name
		annotation.append(annotationName) ;
		
		final String annotationParams = this.getAnnotationParams(parameter.getName(), parameter.getAlias(), parameter.isRequired(), annotationName) ;
	 ...
	 */
	protected String getAnnotationParams(final String parameterName, final String aliasName, final boolean required, final String annotationName)
	{
		final StringBuilder outcome = new StringBuilder() ;
		
		// Set the annotation "value" if it is not a @RequestBody
		boolean valueAnnotationFilled = !ConstantsOutputClientFeign.FEIGN_ANN_REQUEST_BODY.equals(annotationName) ;
		if (valueAnnotationFilled)
		{
			outcome.append(this.setAnnotationValue(parameterName, aliasName)) ;
		...
		if (!ConstantsOutputClientFeign.FEIGN_ANN_PATH_VARIABLE.equals(annotationName))
		{
	 ...
	 */
	private StringBuilder setAnnotationValue(final String parameterName, final String aliasName)
	{
		final StringBuilder outcome = new StringBuilder() ;
		...
		final boolean aliasFilled = aliasName != null && !aliasName.isEmpty() ;
	...
	private String getTypeOutputFile(final Parameter parameter)
	{
		...
		{
			type = ConstantsOutputClientFeign.FEIGN_FORM_DATA_MULTPART ;
		}
