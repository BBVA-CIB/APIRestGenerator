 ...
 * ------------------------------------------------
 */
public class TranslatorGeneratorClientFeignInterfaceRest extends TranslatorGeneratorJavaRestInterfaceBase
{
    /**
     * Logger of the class
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(TranslatorGeneratorClientFeignInterfaceRest.class);
     ...
     */
    public TranslatorGeneratorClientFeignInterfaceRest(final GenerationParameters generationParams,
                                                        final ParsedInfoHandler parsedInfoHandler,
                                                        final String packageName)
    {
        super(ConstantsOutputClientFeign.MODULE_NAME, generationParams, parsedInfoHandler, packageName);
    }
    ...
    public void generateRestHandler(final File destDir) throws APIRestGeneratorException
    {
        ...
        TranslatorGeneratorClientFeignInterfaceRest.LOGGER.info("Generating Feign (client) Rest - Three interfaces: handler, listener and hyxtrix fallback") ;

        // Rest Handler Listener class
        final IGenerator restListenerInterfaceGenerator = new RestListenerClientFeignInterfaceGenerator(destDir, 
        																									this.getGenerationParams(),
        																									this.getParsedInfoHandler()) ;
        restListenerInterfaceGenerator.generate();

        // Rest Handler Generator
        final IGenerator restHandlerInterfaceGenerator = new RestHandlerClientFeignInterfaceGenerator(destDir,
        																			  					  this.getGenerationParams(),
        																			  					  this.getParsedInfoHandler()) ;
        restHandlerInterfaceGenerator.generate();
        
        // Hyxtrix Fallback class
        final IGenerator hystrixFallbackGenerator      = new HystrixFallbackClientFeignInterfaceGenerator(destDir,
        																									  this.getGenerationParams(),
        																									  this.getParsedInfoHandler()) ;
        hystrixFallbackGenerator.generate();

        TranslatorGeneratorClientFeignInterfaceRest.LOGGER.info("Generated Feign (client) Rest - Three interfaces: handler, listener and hyxtrix fallback") ;
    }
    ...
    {
        TranslatorGeneratorClientFeignInterfaceRest.LOGGER.info("Generating Feign (client) pom files for the rest interface project");

        // Generate the pom.xml file
        final IGenerator normalPom = new RestPOMClientFeignInterfaceGenerator(destDir,
                                                                               this.getGenerationParams(),
                                                                               this.getParsedInfoHandler(),
        ...
        final IGenerator artifactoryPom = new RestPOMClientFeignInterfaceGenerator(destDir,
                                                                                    this.getGenerationParams(),
                                                                                    this.getParsedInfoHandler(),
                                                                                    this.getArtifactoryPomName());
        artifactoryPom.generate();

        TranslatorGeneratorClientFeignInterfaceRest.LOGGER.info("Generated Feign (client) pom files for the rest interface project");
    }

    @Override
    public void generateSpecificJavaFramework(final File destDir) throws APIRestGeneratorException
    {
        // Nothing specific to implement with this Feign MVC framework
    }
